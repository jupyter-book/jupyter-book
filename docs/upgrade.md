...
- Modern web features and improved interactivity
- Active development and new features

**Stay on 1.0 if you need:**
...
## Upgrade a Sphinx site to use Jupyter Book

### Translating the Sphinx `toctree` structure into a Jupyter Book Table of Contents

If you've used a standard Sphinx site and you'd like to convert it to Jupyter Book, you'll need to turn the `{toctree}` directives into a [MyST Table of Contents file](xref:guide#toc-format). Here's a simple-enough approach you can follow:

First, generate a MyST Table of Contents for your Sphinx site. You can do so with the following command:
...
This will parse your file structure and generate a Table of Contents structure using the file and folder names.

Next, re-order the files in your generated Table of Contents so that they map onto the nested `{toctree}` structure in your pages. Remove the `{toctree}` directives as you do this.
...
And `index.md` has a `{toctree}` like this:
...
Then it will look like the following in your MyST Table of Contents file:
...
(known-limitations)=
### What are the known limitations in Jupyter Book 2?

Some features from Jupyter Book 1 are still in development for version 2:
